...
_STRINGS: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "cli.examples.chat_interactive": '  # Chat (interactive)\n  r9s chat [bot] --model "$R9S_MODEL"',
        "cli.examples.chat_pipe": '  # Chat (pipe stdin)\n  echo "hello" | r9s chat [bot] --model "$R9S_MODEL"',
        "cli.examples.chat_pipe_image": '  # Chat (pipe image)\n  cat image.png | r9s chat [bot] --model "$R9S_MODEL"',
        "cli.examples.resume": "  # Resume a session\n  r9s chat resume",
        "cli.examples.bots": '  # Bots & commands\n  r9s bot create grammar --system-prompt "You are a grammar assistant, you will translate the user\'s input into authentic English"\n  r9s command create git-commit-msg --prompt $\'generate a commit message based on the following diff\\ndiff\\n!{git diff --staged}\'\n  r9s chat grammar',
