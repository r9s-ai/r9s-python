...
_STRINGS: Dict[str, Dict[str, str]] = {
    "en": {
        ...
        "chat.commands.model": "  /model [name]  Show or switch current model",
        ...
        "chat.msg.current_model": "Current model: {model}",
        "chat.msg.model_switched": "Switched to model: {model}",
        ...
        "chat.err.missing_api_key": "Missing API key: set R9S_API_KEY, put api_key in ~/.r9s/config.toml, or pass --api-key",
        "chat.err.missing_model": "Missing model: set R9S_MODEL, put model in ~/.r9s/config.toml, or pass --model",
        ...
        "set.using_api": "Using API base URL: {url}",
        ...
        "web.err.streamlit_missing": "Web UI dependency 'streamlit' is not installed.\nRun: pip install 'r9s[web]'",
        "web.err.bind_host_failed": "Unable to allocate an available port for host={host!r}. Use --host to specify a bindable address.",
        "web.err.port_out_of_range": "--port must be in the range 0..65535",
        "web.msg.auto_port_allocated": "Automatically allocated available port: {port}",
        "web.msg.port_switched": "Port {preferred_port} is in use; automatically switching to {port} (use --no-auto-port to disable)",
        "web.err.port_range_exhausted": "Port {preferred_port} is in use, and the next 100 ports are also unavailable; use --port to specify one.",
        "web.chat.caption.supported_models": "Only models that support chat conversation protocols are shown",
        "web.chat.caption.no_models": "No models supporting chat conversation protocols were found; you can still enter a custom model id",
        "web.chat.info.select_model": "Select a model before starting the conversation.",
    },
    "zh-CN": {
        "cli.title": "r9s CLI",
        ...
        "chat.commands.model": "  /model [name]  显示或切换当前模型",
        ...
        "chat.msg.current_model": "当前模型：{model}",
        "chat.msg.model_switched": "已切换到模型：{model}",
        ...
        "chat.err.missing_api_key": "缺少 API key：请设置 R9S_API_KEY、在 ~/.r9s/config.toml 中填写 api_key，或传入 --api-key",
        "chat.err.missing_model": "缺少 model：请设置 R9S_MODEL、在 ~/.r9s/config.toml 中填写 model，或传入 --model",
        ...
        "set.using_api": "使用 API base URL：{url}",
        ...
        "web.err.streamlit_missing": "未安装 Web UI 依赖：streamlit。\n请执行：pip install 'r9s[web]'",
        "web.err.bind_host_failed": "无法为 host={host!r} 分配可用端口，请使用 --host 指定一个可绑定的地址。",
        "web.err.port_out_of_range": "--port 必须在 0..65535 范围内",
        "web.msg.auto_port_allocated": "已自动分配可用端口：{port}",
        "web.msg.port_switched": "端口 {preferred_port} 已被占用，自动切换到 {port}（可用 --no-auto-port 禁用）",
        "web.err.port_range_exhausted": "端口 {preferred_port} 已被占用，且后续 100 个端口都不可用；请使用 --port 指定。",
        "web.chat.caption.supported_models": "仅展示支持聊天会话协议的模型",
        "web.chat.caption.no_models": "未获取到支持聊天会话协议的模型列表；可用自定义模型 id",
        "web.chat.info.select_model": "请选择一个模型（Model），然后再开始对话。",
    },
}
