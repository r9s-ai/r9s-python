...
def basic_message():
    ...
    with R9S(api_key=os.getenv("R9S_API_KEY", "")) as r9s:
        ...
        print(f"Usage: input_tokens={res.usage.input_tokens}, output_tokens={res.usage.output_tokens}")
            ...
            messages=[
                {
                    ...
                    "content": [{
                        "type": "text",
                        "text": "Tell me about the pyramids of Egypt."
                    }]
                }
            ],
            max_tokens=500,
...
def parallel_tool_calls():
    ...
    with R9S(api_key=os.getenv("R9S_API_KEY", "")) as r9s:
        ...
        tool_calls = [block for block in res.content if getattr(block, 'TYPE', getattr(block, 'type', None)) == "tool_use"]  # type: ignore
